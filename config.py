...
class Config:
    ...
    REQUIRED = [
        'box_length', 'num_particles', 'equil_steps', 'prod_steps', 'output_interval', 'output_rcut', 'output_rcut_traj',
        'internal_interval', 'seed', 'bias_type', 'avbmc_rate', 'nvt_rate',
        'translation_rate', 'swap_rate', 'max_displacement', 'upper_bonded_cutoff',
        'lower_bonded_cutoff', 'clust_cutoff', 'ff_path', 'input_path', 'kT', 'ratio',
    ...
    def _apply_defaults(self):
        ...
        # Set AVBMC upper bonded cutoff to clust_cutoff by default
        if not 'upper_bonded_cutoff' in self.parameters:
    ...
    def _calculate_concentration(self, box_length, num_particles):
        ...
        # Number of formula units is set by the limiting ion in the ratio
    ...
    def _set_bias(self):
        ...
        bias_type = self.parameters['bias_type']  # always present (defaulted to None)
        if bias_type == 'harmonic':
            ...
            # Bias maps path=None to a zero potential, so the no-file case needs no special handling
    ...
    def _setup_move_types(self):
        ...
        # will need to manually import subsequently added moves
        ...
        # AVBMC and NVT rates are split 50/50 between in->out and out->in moves
