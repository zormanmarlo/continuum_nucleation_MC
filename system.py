...
from utils import PMF, Bias, generate_energy_table
...
def calc_energy(particle_idx, positions, types, box_length, clust_cutoff, counter_cutoff, cluster_size, pmf=None):
        ...
        # currently hardcoded JC and dang, will want to make this more flexible later
        if pmf.ff_type == "dang":
...
class System:
    def __init__(self, l, n, s=11, pmf_path="gale.txt", bias_path=None, upper_cutoff=3.9, lower_cutoff=2.7, clust_cutoff=3.9, counter_cutoff=6.0, max_trans=4.2, target_max=200, center=0, k=0, id=0):
        ...
        # use a linear bias for US with a target cluster size
        if center == 0:
    ...
    def init(self, input_path=None):
        ...
        # put target cluster in middle of the box for US 
    ...
    def find_cluster_around_target(self, target_idx=0):        
        ...
        # commentiong out scipy version for now
        # tree = cKDTree(self.positions, boxsize=self.box_length+0.001)
        visited = set()
        queue = [target_idx]
        cluster = []
        ...
        while queue:
            ...
            if current not in visited:
                visited.add(current)
                cluster.append(current)
                # commenting out scipy version for now
                # neighbors = tree.query_ball_point(self.positions[current], self.clust_cutoff)
    ...
    # CURRENTLY BUGGED, BUT ONLY CALL ONCE AT BEGINNING OF SIMULATION
        self.energy = 0.0
        for i in range(self.num_particles):
    ...
    def translation(self, particle_idx):
        ...
        old_energy = calc_energy(particle_idx, self.positions, self.types, self.box_length, self.clust_cutoff, self.counter_cutoff, part_cluster_size, self.pmf)
        ...
        new_energy = calc_energy(particle_idx, self.positions, self.types, self.box_length, self.clust_cutoff, self.counter_cutoff, part_cluster_size, self.pmf)
    ...
    def inout_AVBMC(self, anchor_idx):
        ...
        old_energy = calc_energy(target_idx, self.positions, self.types, self.box_length, self.clust_cutoff, self.counter_cutoff, part_cluster_size, self.pmf)
        ...
        new_energy = calc_energy(target_idx, self.positions, self.types, self.box_length, self.clust_cutoff, self.counter_cutoff, part_cluster_size, self.pmf)
        delta_energy = new_energy - old_energy
        self.energy += delta_energy
        self.bias_energy += bias_energy
