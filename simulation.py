...
class Simulation:
    def __init__(self, config_file, jobname, ID=0, path=".", multi_inputs=False):
        ...
        if self.config.parameters['output_detailed_balance']:
        ...
        if hasattr(self, 'detailed_balance_file'):
            db_headers = ' '.join(name for name in self.system.move_names)
            self._append_line(self.detailed_balance_file, f'# step {db_headers}')
    ...
    def write_output(self, step):
        ...
        # Write rcut output (target cluster only, when larger than monomer)
        rcut_minimum = 1
        if hasattr(self, 'rcut_file'):
            ...
            if len(target_clust) > rcut_minimum:
                ...
                with open(self.rcut_traj_file, 'a') as f:
                    ...
                    for i, particle in enumerate(translated_positions):
                        atom_type = 'H' if particle_types[i] == 0 else 'O'
                        f.write(f'{atom_type} {particle[0]:>6.2f} {particle[1]:>6.2f} {particle[2]:>6.2f}\n')

        # Write detailed balance output (LJ-specific)
        if hasattr(self, 'detailed_balance_file'):
            ...
            self._append_line(self.detailed_balance_file, f'{step} {db_str}')

        # Stats become cumulative (no per-window reset; matches typemoves)
...
if __name__ == "__main__":
    ...
    try:
        ...
        shutil.copy(args.config, f"{args.path}/{args.jobname}/config.yaml")
